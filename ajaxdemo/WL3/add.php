...
function search(){
	...
	$dian= $_GET["dian"];
	if(is_numeric($wm)){
		if($dian=="a"){$link_a="https://uniqlo.tmall.com/search.htm?";}else{$link_a="http://www.uniqlo.cn/search.htm?";}
		echo'{"success":true,"xianding":true,"text":"' . $wm. '","msg":"' . $link_a. '"}';
			...
			if(strpos($wm, "限定")){
				...
				if($result = $mysqli->query($sql)){
					...
					echo'{"success":true,"xianding":true,"text":"' . $text. '","msg":"' . $row[0]. '' . $fu. '"}';
				}else{
		...
		if($result = $mysqli->query($sql)){
		$row=$result->fetch_row();
			echo'{"success":true,"xianding":false,"msg":"' . $row[0]. '"}';
		}else{
			echo '{"success":false,"msg":"失败"}';};
	}
